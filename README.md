...
- `redact-export.py` - strips ballots, tokens and email addresses from a KV export before it is committed. The backup workflow calls it; **never commit a raw export.** Its verifier checks the output against `FORBIDDEN_IN_OUTPUT`, a list **deliberately duplicated** from `DROP_PREFIXES`/`DROP_EXACT` - do not refactor them into one. It used to verify against the same constants it was verifying, which meant deleting one line (`qv:ballots:`) published every per-voter ballot into this public repo and exited 0 printing "nothing dropped". `scripts/test-redact-export.mjs` covers that exact case and runs in the SessionStart hook.
- `check-generated.mjs` - fails the build when a committed generated file no longer matches its source. Runs inside `validate.mjs` and never leaves the tree modified. `/recordings/index.json` and `/recordings.txt` are the surface agents are told to read, so a stale one is a wrong answer served confidently.
- `check-recheck.mjs` - fails the build when a time-bound claim is past its `<!-- RECHECK YYYY-MM-DD: ... -->` date. Runs inside `validate.mjs`. Stale docs were the entire reason for PRs #669/#670; this makes the repo refuse to look healthy while a dated claim is unverified. The fix on failure is to re-verify, not to bump the date.
