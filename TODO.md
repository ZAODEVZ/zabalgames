...
- **Farcaster Mini App hardening** - research + first implementation pass done 2026-06-06. Splash fix shipped earlier (PR #127: exact SDK pin `0.1.10` + `esm.sh` preconnect on all 27 pages). Status of the four hardening items:
  - **[P1] Self-host the SDK - SHIPPED (loader + vendored files).** `assets/miniapp.js` loads self-host-first (`/assets/vendor/miniapp-sdk-0.1.10.js`) with esm.sh as fallback, via a null-safe dynamic-import loop. The vendored bundle is now committed: `assets/vendor/miniapp-sdk-0.1.10.js` (esm.sh `?bundle` output) plus `assets/vendor/buffer.mjs` (the SDK's only external dep, vendored and its import path rewritten to `./buffer.mjs` so the module resolves entirely same-origin). Verified self-contained (no absolute/cross-origin imports) and that it links + exports `sdk` via `node --check` + an ESM link test. This flips the app to same-origin - the actual cure for the stuck splash. Remaining: confirm in a real Farcaster client (the CDN fallback masks a bad vendored file, so exercise the same-origin path).
