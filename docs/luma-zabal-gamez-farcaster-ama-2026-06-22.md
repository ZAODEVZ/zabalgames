...
4. Location: Link Meeting -> paste the Farcaster audio-space link
...
7. Host: BetterCallZaal (The ZAO), co-invite @farcaster
8. Add the registration question below (optional) so RSVPs can carry a question
9. Publish, then post the questions cast in /zabal and send me the event URL
...
Live on the homepage schedule now, with RSVP pointed at the ZAO calendar in the interim.
Send me the Lu.ma event URL and I will swap `luma_url` to the specific event.
