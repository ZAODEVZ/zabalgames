...
- **A say in the next adoptable-project list** - Season 2 selections informed by mentor feedback
...
Pull from /about.html roster + the 8 mentor slots noted there. Personalize the "why you" line per recipient.

| # | Target | Handle | Why them (the angle for the "why you" line) |
|---|---|---|---|
...
## Backup roster (if any of top 8 say no)

| # | Target | Why backup not primary |
|---|---|---|
...
| Day | Action |
|---|---|
| Sun May 25 (today after sundown) | Send DMs to top 3 (Jordan + Adrian + Tyler). Most likely to say yes fast. |
| Mon May 26 | Send DMs to Hurric4n3Ike + Sam + Arthur (WaveWarZ trio). |
| Tue May 27 | Send DMs to kmac.eth + Joshua.eth. |
| Wed May 28 | Follow up anyone who hasn't replied. Send backup list 9-12 to fill remaining slots. |
| Thu May 29 | Lock the confirmed mentors into `data/mentors.json` - I add each entry as you confirm. |
| Fri May 30 | /about.html mentor section reflects the locked roster. Mentor count on hero ticker updates. |
| Sat May 31 | Final check. Launch cast names the locked mentors. |

## What you can paste right now

The fastest unblock is the top 3 - Jordan, Adrian, Tyler. They're already in the conversation orbit and most likely to say yes. Suggested copy for each:
...
### To Tyler (already in conversation)
...
extra ~6 hrs in August. worth it?
```

## After each yes

I add an entry to `data/mentors.json` with the structure already documented there. The `/mentor?handle=<their-handle>` page auto-renders. The /about.html mentor section gets a live update. The hero counter ticks down the open-slots count.

Send each confirmation DM directly to me and I'll commit + push within minutes.
