...
//
// Run it from a machine (or cron) WITH outbound network:
//   node scripts/resolve-pfps.mjs
//   NEYNAR_API_KEY=xxxx node scripts/resolve-pfps.mjs   (more reliable username lookups)
//
// Strategy per handle: Haatz by_username (keyless) -> Neynar by_username (if key) ->
// Haatz search. A handle that resolves nowhere is left out, so the tile falls back to text.
