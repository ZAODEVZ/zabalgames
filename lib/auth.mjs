...
export async function verifyAdmin(req, domain) {
  ...
  if (ADMIN_KEY) {
    ...
    let diff = token.length ^ ADMIN_KEY.length;
    for (let i = 0; i < Math.max(token.length, ADMIN_KEY.length); i++) {
      diff |= (token.charCodeAt(i) || 0) ^ (ADMIN_KEY.charCodeAt(i) || 0);
    }
    if (diff === 0) return { ok: true, key: true };
  }
  return { ok: false };
}
