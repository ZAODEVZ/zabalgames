...
// The Mini App's domain - the Quick Auth `aud` every endpoint verifies against.
// Single source of truth: endpoints import this instead of re-declaring it, so a
// domain change is one edit, not 13 (and no endpoint can drift to a stale value).
export const DOMAIN = 'zabalgamez.com';
...
// Length-safe constant-time string compare. The single shared implementation for
// every shared-secret check in api/ - previously copy-pasted into five files plus an
// inline variant below, which is how one endpoint drifts to `===` without anyone
// noticing. Returns false for empty/missing values so a caller can never authorize
// on two blank strings.
...
// Admin allowlist by Farcaster FID. FIDs are public identifiers, so hardcoding is
// safe (unlike a shared key in a public repo). Add an admin = add their FID here.
// The ADMIN_FIDS env var (comma-separated) extends the list without a code change.
const ADMIN_FIDS = new Set([19640, 1057869]); // zaal, imanafrikah (Iman - ZAO Devs lead, for moderation testing)

export function isAdminFid(fid) {
  const n = Number(fid);
  if (!n) return false;
  if (ADMIN_FIDS.has(n)) return true;
  return (process.env.ADMIN_FIDS || '').split(',').some((s) => Number(s.trim()) === n);
}

// Verify an admin request. Accepts EITHER a Farcaster Quick Auth JWT whose FID is on
// the allowlist (primary path - sign in with Farcaster, no shared secret) OR a Bearer
// ADMIN_KEY (optional fallback for desktop/scripts, only if that env is set). Returns
// { ok, fid? }. verifyQuickAuth throws fast on a non-JWT token, so the key fallback
// adds no network cost. Both paths fail closed.
export async function verifyAdmin(req, domain) {
  const auth = req.headers.get('authorization') || '';
  const token = auth.startsWith('Bearer ') ? auth.slice(7).trim() : '';
  if (!token) return { ok: false };
  try {
    const fid = await verifyQuickAuth(token, domain || DOMAIN);
    return isAdminFid(fid) ? { ok: true, fid } : { ok: false };
  } catch (_) { /* not a JWT - fall through to the key fallback */ }
  const ADMIN_KEY = process.env.ADMIN_KEY || '';
  if (ADMIN_KEY && timingEq(token, ADMIN_KEY)) return { ok: true, key: true };
  return { ok: false };
}
