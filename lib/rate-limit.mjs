...
export class RateLimiter {
  constructor(kvUrl, kvToken) {
    ...
    // unref'd where the runtime supports it, so a timer can never hold a process open - this
    // module is imported by tests and scripts, not only by the edge runtime.
  ...
  async checkLimit(ip, endpoint, opts = {}) {
    ...
    // If KV is not configured, fall back to the in-memory limiter - do NOT allow everything.
    //
    // This used to `return true`, i.e. an unconfigured environment had NO rate limiting at all,
    // on the four endpoints that accept writes. That contradicted this class's own stated
    // design: the constructor comment says the in-memory map exists "to degrade gracefully
    // instead of failing open", and the KV-error path below already does exactly that. Only the
    // unconfigured path failed open, which is the worse of the two cases - a missing env var is
    // silent and can persist for a whole deployment, where a KV outage is transient.
    //
    // The fallback still allows normal traffic (perMinute per IP per endpoint), so local
    // development and preview deploys are unaffected unless they are actually hammering an
    // endpoint. A rate limiter that no-ops is not graceful, it is absent.
    if (!this.kvUrl || !this.kvToken) return this.checkInMemoryLimit(ip, endpoint, perMinute);
